...
def _avg_crowd_rating(d: dict) -> float:
    """Average crowd rating for a drama dict."""
...
def list_dramas_for_charts() -> dict:
    """Return chart data: most watched (algorithm), top rated (all-time), trending.

    Most Watched uses a blended score:
      - 40% view_count (normalized)
      - 30% avg crowd rating (normalized to 0-1)
      - 30% recency (exponential decay, half-life = 10 minutes)
    This simulates a real recommendation algorithm where popular + well-rated
    + recent content surfaces to the top.

    Top Rated is purely by avg crowd rating across all dramas ever.

    Both always return exactly 5 items (padded from all_dramas if needed).
    """
    all_dramas = list_dramas()
    if not all_dramas:
        return {"most_watched": [], "top_rated": [], "trending": []}

    now = time.time()
    half_life = 600  # 10 minutes

    # Normalize view counts
    max_views = max((d.get("view_count", 0) for d in all_dramas), default=1) or 1

    def most_watched_score(d):
        views_norm = d.get("view_count", 0) / max_views
        rating_norm = _avg_crowd_rating(d) / 5.0
        age_sec = now - d.get("created_at", 0)
        recency = 2 ** (-age_sec / half_life)  # exponential decay
        return 0.4 * views_norm + 0.3 * rating_norm + 0.3 * recency

    most_watched = sorted(all_dramas, key=most_watched_score, reverse=True)

    # Top rated = all-time, by avg crowd rating desc (tie-break by views)
    top_rated = sorted(
        all_dramas,
        key=lambda d: (_avg_crowd_rating(d), d.get("view_count", 0)),
        reverse=True,
    )
    ...
    # Always return exactly 5 for most_watched and top_rated
    # If fewer than 5, pad from all_dramas (avoiding duplicates)
    def pad_to_5(primary: list[dict], fallback: list[dict]) -> list[dict]:
        result = primary[:5]
        seen = {d.get("id") for d in result}
        for d in fallback:
            if len(result) >= 5:
                break
    ...
    return {
        "most_watched": pad_to_5(most_watched, all_dramas),
        "top_rated": pad_to_5(top_rated, all_dramas),
        "trending": trending,
    }
...
FIRST_NAMES = [
    ...
    "Ariya", "Bao", "Chantara", "Dara", "Hien", "Kiet", "Linh", "Mai",
    "Niran", "Phuong", "Sakda", "Tala", "Vien", "Wattana", "Aroha", "Koa",
    # Celtic / Irish
    "Aoife", "Cillian", "Niamh", "Declan", "Saoirse", "Fionn", "Caoimhe", "Ronan",
]
...
LAST_NAMES = [
    ...
    # Southeast Asian / Pacific
    "Nguyen", "Tran", "Pham", "Srisai", "Ratanakul", "Suwan", "Dela Cruz", "Santos",
    # Celtic / Irish
    "O'Sullivan", "Murphy", "Byrne", "O'Connor", "Kelly", "Walsh", "Doyle", "Gallagher",
]
